...
test("S3 삭제 실패 작업을 남기고 다음 실행에서 정리한다", async () => {
  const tasks: S3CleanupTask[] = [
    { cleanupId: 1n, objectKey: "audio-guides/rain.mp3", attempts: 0 },
  ];
  ...
  const repository: S3CleanupTaskRepository = {
    ...
    async complete(cleanupId) {
      const index = tasks.findIndex((task) => task.cleanupId === cleanupId);
      if (index >= 0) tasks.splice(index, 1);
    },
    async recordFailure(cleanupId, error) {
      const task = tasks.find((candidate) => candidate.cleanupId === cleanupId);
